...
export async function saveImageToFolder(dataUrlOrBlob, category, filename) {
  ...
  // احتیاط اضافه: اگه این اسم فایل قبلاً (موفق یا ناموفق) resolve و cache شده
  // بود، همینجا باطلش می‌کنیم تا بار بعد که کسی خواستش، از دیسک/IndexedDB
  // واقعیِ تازه‌نوشته‌شده بخونه، نه یه src قدیمی (که ممکنه اصلاً به همین فایل
  // اشاره نکنه، مثلاً وقتی یه فایل قبلی با همین اسم overwrite شده)
  resolvedSrcCache.delete(`${category}/${finalFilename}`);
...
export async function getImageSrc(filename, category) {
  ...
  // باگ بزرگ پیدا‌شده توسط کاربر: قبلاً نتیجه‌ی ناموفق (null — یعنی «فایل پیدا
  // نشد») هم دقیقاً مثل نتیجه‌ی موفق cache می‌شد، برای همیشه تا آخر عمر سشن.
  // یعنی اگه یه فایل یه‌بار (حتی زودتر از این‌که واقعاً روی دیسک ساخته/کپی بشه)
  // resolve می‌شد و شکست می‌خورد، حتی بعد از این‌که فایل واقعی بعداً ساخته
  // می‌شد (آپلود دستی، auto-link، یا کپی دستی کاربر با فایل‌منیجر)، دیگه
  // هیچ‌وقت دوباره چک نمی‌شد — همیشه همون null قدیمی برمی‌گشت (چرا حذف‌کردن
  // فایل مشکل‌دار و امتحان یه اسم جدید درست کار می‌کرد: چون کلید کش‌ش قبلاً
  // هیچ‌وقت query نشده بود). الان فقط نتیجه‌ی *موفق* cache می‌شه؛ شکست هربار
  // دوباره از دیسک چک می‌شه (هزینه‌ش کمه، ولی جلوی این کلاس کامل از باگ‌های
  // «انگار فایل رو نمی‌شناسه» رو می‌گیره).
  if (src !== null) resolvedSrcCache.set(cacheKey, src);
  return src;
}
